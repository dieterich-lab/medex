...
function setup_datatable(
    ...
    const datatable = new Datatable(`#${table_id}`, {
        ...
        ordering: false,
        ajax: {
            url: `${BASE_URL}/basic_stats/${descriptor.name}`,
...
function get_download_link(params: EntityStatsParameters, descriptor: TabDescriptor): string {
    ...
    return `${BASE_URL}/basic_stats/${ name }_csv?${ search_params }`;
}

// eslint-disable-next-line @typescript-eslint/no-unused-vars
function format_number(data: unknown, _type: unknown, _row: unknown) {
    const cooked_data = Number(data)
    if ( isNaN(cooked_data) ) {
        return 'n/a'
    } else {
        return Math.round(100 * cooked_data) / 100;
    }
}

export {EntityStatsResults, type EntityStatsParameters, type EntityStatsResultsProps}
