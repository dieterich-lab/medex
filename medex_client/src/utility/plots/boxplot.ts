...
import {AbstractPlot, AbstractPlotParameters, SimpleRawPlotData} from "./abstract_plot";
...
interface RawBoxplotData {
    table_json: SimpleRawPlotData,
    image_json: SimpleRawPlotData,
}

class BoxplotDescriptor extends AbstractPlot<BoxplotParameters, RawBoxplotData> {
    ...
    get_plot_data_from_response_data(data: RawBoxplotData) {
        return [
            {name: 'table', data: data.table_json.data, layout: data.table_json.layout},
            {name: 'plot', data: data.image_json.data, layout: data.image_json.layout},
        ];
    }
}

export {BoxplotDescriptor, type BoxplotParameters};
