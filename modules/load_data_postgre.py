...
def get_categorical_entities(rdb):
    sql = """Select "Key" from name_type2 where "type" = 'String' """
...
def number(r):
    ...
    sql = """SELECT COUNT (*) FROM examination_categorical2"""
    n = pd.read_sql(sql, r)
...
def get_values(entity, r):
    ...
    sql = """SELECT  "Patient_ID","Key","Value" FROM examination_numerical2 WHERE "Key" IN ({})""".format(entity_fin)
...
def get_cat_values(entity,r):
    ...
    sql = """SELECT  "Patient_ID","Key","Value" FROM examination_categorical2 WHERE "Key" IN ({}) """.format(entity_fin)
    df = pd.read_sql(sql, r)
...
def get_num_cat_values(entity_num,entity_cat,r):
    ...
    sql = """SELECT  en."Patient_ID",en."Value" as "{0}",ec."Value" as "{3}" FROM examination_numerical2 as en 
...
def get_numeric_entities(rdb):
    sql = """Select "Key" from name_type2 where type = 'Double'"""
    df= pd.read_sql(sql, rdb)
